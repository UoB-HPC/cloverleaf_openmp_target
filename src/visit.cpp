...
void visit(global_variables &globals, parallel_ &parallel) {
	...
	for (int tile = 0; tile < globals.config.tiles_per_chunk; ++tile) {
		if (globals.chunk.task == parallel.task) {
			...
			auto hm_density0 = globals.chunk.tiles[tile].field.density0;

			for (int k = globals.chunk.tiles[tile].info.t_ymin + 1;
			     k <= globals.chunk.tiles[tile].info.t_ymax + 1; ++k) {
				for (int j = globals.chunk.tiles[tile].info.t_xmin + 1;
				     j <= globals.chunk.tiles[tile].info.t_xmax + 1; ++j) {
					u << std::scientific << std::setprecision(3) << hostidx2(hm_density0, j, k) << std::endl;
				}
			}

			u << "energy 1 " << nxc * nyc << " double" << std::endl;

			auto hm_energy0 = globals.chunk.tiles[tile].field.energy0;

			for (int k = globals.chunk.tiles[tile].info.t_ymin + 1;
			     k <= globals.chunk.tiles[tile].info.t_ymax + 1; ++k) {
				for (int j = globals.chunk.tiles[tile].info.t_xmin + 1;
				     j <= globals.chunk.tiles[tile].info.t_xmax + 1; ++j) {
					u << std::scientific << std::setprecision(3) << hostidx2(hm_energy0, j, k) << std::endl;
				}
			}
			...
			auto hm_pressure = globals.chunk.tiles[tile].field.pressure;

			for (int k = globals.chunk.tiles[tile].info.t_ymin + 1;
			     k <= globals.chunk.tiles[tile].info.t_ymax + 1; ++k) {
				for (int j = globals.chunk.tiles[tile].info.t_xmin + 1;
				     j <= globals.chunk.tiles[tile].info.t_xmax + 1; ++j) {
					u << std::scientific << std::setprecision(3) << hostidx2(hm_pressure, j, k) << std::endl;
				}
			}

			u << "viscosity 1 " << nxc * nyc << " double" << std::endl;

			auto hm_viscosity = globals.chunk.tiles[tile].field.viscosity;

			for (int k = globals.chunk.tiles[tile].info.t_ymin + 1;
			     k <= globals.chunk.tiles[tile].info.t_ymax + 1; ++k) {
				for (int j = globals.chunk.tiles[tile].info.t_xmin + 1;
				     j <= globals.chunk.tiles[tile].info.t_xmax + 1; ++j) {
					double temp = (std::fabs(hostidx2(hm_viscosity, j, k)) > 0.00000001) ? hostidx2(hm_viscosity, j, k)
					                                                                     : 0.0;
			...
			auto hm_xvel0 = globals.chunk.tiles[tile].field.xvel0;

			for (int k = globals.chunk.tiles[tile].info.t_ymin + 1;
			     k <= globals.chunk.tiles[tile].info.t_ymax + 1 + 1; ++k) {
				for (int j = globals.chunk.tiles[tile].info.t_xmin + 1;
				     j <= globals.chunk.tiles[tile].info.t_xmax + 1 + 1; ++j) {
					double temp = (std::fabs(hostidx2(hm_xvel0, j, k)) > 0.00000001) ? hostidx2(hm_xvel0, j, k) : 0.0;
					u << std::scientific << std::setprecision(3) << temp << std::endl;
				}
			}
			u << "y_vel 1 " << nxv * nyv << " double" << std::endl;

			auto hm_yvel0 = globals.chunk.tiles[tile].field.yvel0;

			for (int k = globals.chunk.tiles[tile].info.t_ymin + 1;
			     k <= globals.chunk.tiles[tile].info.t_ymax + 1 + 1; ++k) {
				for (int j = globals.chunk.tiles[tile].info.t_xmin + 1;
				     j <= globals.chunk.tiles[tile].info.t_xmax + 1 + 1; ++j) {
					double temp = (std::fabs(hostidx2(hm_yvel0, j, k)) > 0.00000001) ? hostidx2(hm_yvel0, j, k) : 0.0;
					u << std::scientific << std::setprecision(3) << temp << std::endl;
				}
			}
			u.close();
