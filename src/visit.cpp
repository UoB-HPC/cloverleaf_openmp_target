		...
		if (globals.chunk.task == parallel.task) {
			...
			for (int k = globals.chunk.tiles[tile].info.t_ymin + 1;
				...
				for (int j = globals.chunk.tiles[tile].info.t_xmin + 1;
					...
					u << std::scientific << std::setprecision(3) << idx2(hm_density0, j, k) << std::endl;
				}
			}
			...
			for (int k = globals.chunk.tiles[tile].info.t_ymin + 1;
				...
				for (int j = globals.chunk.tiles[tile].info.t_xmin + 1;
				     j <= globals.chunk.tiles[tile].info.t_xmax + 1; ++j) {
					u << std::scientific << std::setprecision(3) << idx2(hm_energy0, j, k) << std::endl;
				}
			}
			...
			for (int k = globals.chunk.tiles[tile].info.t_ymin + 1;
				...
				for (int j = globals.chunk.tiles[tile].info.t_xmin + 1;
				     j <= globals.chunk.tiles[tile].info.t_xmax + 1; ++j) {
					u << std::scientific << std::setprecision(3) << idx2(hm_pressure, j, k) << std::endl;
				}
			}
			...
			for (int k = globals.chunk.tiles[tile].info.t_ymin + 1;
				...
				for (int j = globals.chunk.tiles[tile].info.t_xmin + 1;
				     j <= globals.chunk.tiles[tile].info.t_xmax + 1; ++j) {
					double temp = (std::fabs(idx2(hm_viscosity, j, k)) > 0.00000001) ? idx2(hm_viscosity, j, k)
					                                                                 : 0.0;
			...
			for (int k = globals.chunk.tiles[tile].info.t_ymin + 1;
				...
				for (int j = globals.chunk.tiles[tile].info.t_xmin + 1;
				     j <= globals.chunk.tiles[tile].info.t_xmax + 1 + 1; ++j) {
					double temp = (std::fabs(idx2(hm_xvel0, j, k)) > 0.00000001) ? idx2(hm_xvel0, j, k) : 0.0;
					u << std::scientific << std::setprecision(3) << temp << std::endl;
				}
			}
			...
			for (int k = globals.chunk.tiles[tile].info.t_ymin + 1;
				...
				for (int j = globals.chunk.tiles[tile].info.t_xmin + 1;
				     j <= globals.chunk.tiles[tile].info.t_xmax + 1 + 1; ++j) {
					double temp = (std::fabs(idx2(hm_yvel0, j, k)) > 0.00000001) ? idx2(hm_yvel0, j, k) : 0.0;
					u << std::scientific << std::setprecision(3) << temp << std::endl;
				}
			}
			u.close();
